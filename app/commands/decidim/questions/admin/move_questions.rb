      ...
      class MoveQuestions < Rectify::Command
        ...
        def move_questions
          ...
          references = Decidim::Questions::Question.where("reference ~* ?", prefix + '\d+$')
                                                   .where(component: form.target_component, state: %w(evaluating accepted))
                                                   .pluck(:reference)
        ...
        def manage_custom_references
          prefix = form.target_component.name[Decidim.config.default_locale.to_s].capitalize[0]
          current_ref = Decidim::Questions::Question.where(component: form.target_component, state: %w(evaluating accepted)).order(published_at: :desc).first.reference
          current_index = current_ref.split(prefix).last
          current_index = /\A\d+\Z/.match?(next_short_ref) ? next_short_ref.to_i + 1 : 1

          transaction do
            questions.each do |question|
              next unless !question.emendation? && %w(evaluating accepted).include?(question.state)
        ...
        def notify
          questions.each do |question|
            next unless question.coauthorships.any?
