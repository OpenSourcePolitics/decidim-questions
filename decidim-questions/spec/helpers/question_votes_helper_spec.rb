  ...
  module Questions
    describe QuestionVotesHelper do
      ...
      before do
        ...
        allow(helper).to receive(:current_component).and_return(question_component)
      ...
      describe "#vote_button_classes" do
        it "returns small buttons classes from questions list" do
          expect(helper.vote_button_classes(true)).to eq("card__button button--sc")
        end

        it "returns expanded buttons classes if it's not from questions list'" do
          expect(helper.vote_button_classes(false)).to eq("expanded button--sc")
        end
      end

      describe "#votes_count_classes" do
        it "returns small count classes from questions list" do
          expect(helper.votes_count_classes(true)).to eq(number: "card__support__number", label: "")
        end

        it "returns expanded count classes if it's not from questions list'" do
      ...
      describe "#remaining_votes_count_for" do
        it "returns the remaining votes for a user based on the component votes limit" do
          question = create(:question, component: question_component)
          create(:question_vote, author: user, question: question)
